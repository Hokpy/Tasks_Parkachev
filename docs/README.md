...
Учебный проект по летней практике (3 курс, специальность 09.02.07).  
Приложение для управления задачами на **C# + WPF** с архитектурой **MVVM**.
...
2. В Solution Explorer убедиться, что **TaskManager.WPF** установлен как Startup Project  
   (жирный шрифт в дереве; если нет — ПКМ → *Set as Startup Project*)
...
- **RelayCommand** — реализация `ICommand` для привязки кнопок к методам ViewModel

---

## 👤 Автор

Студент группы _____, специальность 09.02.07  
